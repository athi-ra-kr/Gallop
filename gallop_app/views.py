...
def all_students_view(request):
    ...
    # Get all profiles to match database data (phone/premium) with Firebase data
    ...
    for user in firebase_users:
        ...
        merged_students.append({
            ...
            # 🔥 Pass the premium status to the template
            "is_premium": profile.is_premium if profile else False,
        })

    return render(request, 'students_list.html', {'students': merged_students})

# 🔥 Ensure this function exists to handle the click
@user_passes_test(is_admin, login_url='admin_login')
def toggle_premium(request, email):
    # Get or create profile so even new Firebase users can be upgraded
    student, created = StudentProfile.objects.get_or_create(email=email)
    
    # Toggle boolean
    student.is_premium = not student.is_premium
    student.save()

    messages.success(request, f"Status updated for {email}")
...
class ThinkBellQuestionAPI(APIView):
    ...
    def get(self, request):
        ...
        # 🔥 ✅ ADD COMPLETION + REDO LOGIC (ONLY ADDITION)
        attempted = StudentAnswerRecord.objects.filter(
            student=student,
            slide_question__section=section
        ).values("slide_question").distinct().count()
        ...
        return Response({
            ...
            "completed": completed,   # ✅ show completed
            "redo": completed,        # ✅ allow redo
...
class ThinkBellSectionAPI(APIView):
    ...
    def get(self, request):
        ...
        # 🔹 Get email (optional now)
        email = request.GET.get("email")

        # 🔹 If no email → use default test user
        if not email:
...
@user_passes_test(is_admin, login_url='admin_login')
def update_quiz_club_section(request, pk):
    if request.method == "POST":
        data = json.loads(request.body)
        section = get_object_or_404(AppSection, pk=pk, section_type='QC')
...
def update_thinkbell_section(request, pk):
    import json
    if request.method == "POST":
        data = json.loads(request.body)
        section = get_object_or_404(AppSection, pk=pk, section_type='TB')
        section.name = data.get('name')
        section.save()
        return JsonResponse({'success': True})
    
def update_news_section(request, pk):
    import json
    if request.method == "POST":
        data = json.loads(request.body)
        section = get_object_or_404(AppSection, pk=pk, section_type='NB')
        section.name = data.get('name')
        section.save()
        return JsonResponse({'success': True})
...
def filter_sections_by_plan(student, section_type):
    from .models import AppSection

    if student.is_premium:
        return AppSection.objects.filter(section_type=section_type)
    else:
        return AppSection.objects.filter(section_type=section_type, is_premium=False)
